...
class Underwater_navigation():
    def __init__(self, scene_name, visibility,
                 depth_prediction_model, adaptation, randomization, rank, HIST, start_goal_pos=None, training=True):
        ...
        unity_env = UnityEnvironment(os.path.abspath("./") + "/underwater_env/" + scene_name + '_' + visibility,
    ...
    def reset(self):
        ...
        # cv2.imwrite("img_rgb_step" + str(self.step_count) + ".png",
        #             256 * cv2.cvtColor(obs_img_ray[0] ** 0.45, cv2.COLOR_RGB2BGR))
        # cv2.imwrite("img_depth_pred_step"+str(self.step_count)+".png", 256 * self.obs_preddepths[0])
    ...
    def step(self, action):
        ...
        if obstacle_distance < 0.5 or np.abs(obs_goal_depthfromwater[3]) < 0.24 or obstacle_distance_vertical < 0.12:
            ...
            if self.training == False:
                my_open = open(os.path.join(assets_dir(), 'learned_models/test_pos.txt'), "a")
                data = ["failed", " ", str(self.step_count), " "]
                for element in data:
                    my_open.write(element)
                my_open.close()
        else:
        ...
        if self.training:
            ...
            if obs_goal_depthfromwater[0] < 0.8:
                ...
                if self.training == False:
                    my_open = open(os.path.join(assets_dir(), 'learned_models/test_pos.txt'), "a")
                    data = ["success", " ", str(self.step_count), " "]
                    for element in data:
                        my_open.write(element)
                    my_open.close()
            else:
