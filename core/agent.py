    ...
    while num_steps < min_batch_size:
        ...
        if running_state is not None:
            ...
            img_depth, goal, ray, hist_action = \
                img_depth.astype(np.float64), goal.astype(np.float64), ray.astype(np.float64), hist_action.astype(np.float64)
        reward_episode = 0
        ...
        for t in range(10000):
            ...
            if running_state is not None:
                ...
                _, next_goal, next_ray_0 = running_state(next_img_depth, next_goal, next_ray[:, :1])
                next_ray = np.hstack((next_ray_0, next_ray[:, 1:]))
                ...
                next_img_depth, next_goal, next_ray, next_hist_action = next_img_depth.astype(np.float64),\
                                            next_goal.astype(np.float64), next_ray.astype(np.float64), next_hist_action.astype(np.float64)
